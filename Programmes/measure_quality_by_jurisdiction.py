...
for ident in manualdocs:
        ...
        for i in manualentities:
            ...
            if i not in manualentities_without_duplicates or i[1] == 'frais_irrepetibles':
                ...
                print(i)
            if i[1] == 'juridiction' and decision_jurisdiction == "":    
                decision_jurisdiction = i[0]
                #counting the number of decisions per jurisdiction
                AllJur[decision_jurisdiction] += 1
        print("auto")
        ...
        for me in manualentities_without_duplicates:
            ...
            if me not in automaticentities_without_duplicates:
                ...
                FalseNegativeJur[decision_jurisdiction] += 1
                ...
                CorrectEntity[meE] += 1
                CorrectType[meT] += 1
                CorrectJur[decision_jurisdiction] += 1
        ...
        for ae in automaticentities_without_duplicates:
            ...
            if ae not in manualentities_without_duplicates:
                ...
                FalsePositiveJur[decision_jurisdiction] += 1
...
for j in AllJur:
    ...
    fm = 0
    c = CorrectJur[j]
    m = FalseNegativeJur[j]
    f = FalsePositiveJur[j]
    d = AllJur[j]
    if c+m > 0:
        r = c/(c+m)
    if c+f > 0:
    ...
    if r != 0 or p != 0:
        fm = 2*r*p/(r+p)
    print("Jurisdiction: %s, f-measure = %.2f, number of decisions = %s" % (j, fm, d))
    nb_jur += 1
print(nb_jur)